...
Este projeto é acompanhado pelo Mentor de Projetos. Slug: `leitoa-assada`. O estado executivo vive em `.project-mentor/project.yaml` e o histórico em `.project-mentor/sessions/`. **Nunca edite esses arquivos à mão**: toda escrita passa pelo CLI `mentor`.
...
2. Rode `mentor project leitoa-assada --brief` e leia a última sessão em `.project-mentor/sessions/`.
...
4. Pergunte o objetivo só se não estiver claro. Depois rode `mentor start leitoa-assada --objective "..." --agent <claude-code|codex|antigravity|copilot>`.
...
- Ação nova: `mentor action add leitoa-assada --title "..."`. Concluir: `mentor action done leitoa-assada <act-id> --level implemented|tested`. Bloqueio: `mentor blocker add leitoa-assada --description "..."`.
...
2. Rode `mentor close leitoa-assada --from <rascunho.md>` e mostre o resultado da validação. Se falhar, mostre o erro e **não** finja sucesso.
